...
{
	nRF24L01_WriteRead (W_REGISTER, FLUSH_TX, Buffer, 0);//pruebo cambiar de lectura a escritura
	nRF24L01_WriteRead (W_REGISTER, W_TX_PAYLOAD, Buffer, len);


	//P2OUT	|=	 BIT0;											// Start transmission
	P1OUT	|=	 BIT1;											// Start transmission

	Delay_WDT ( RetardoSeguridad );
	//while ( (P2IN & BIT2) == BIT2  && !( IFG1 & WDTIFG ) );		// Wait until the package is sent or TimeOut
	while ( (P1IN & BIT3) == BIT3  && !( IFG1 & WDTIFG ) );		// Wait until the package is sent or TimeOut
	WDTCTL = WDTPW + WDTHOLD;     								// Stop watchdog timer

	//P2OUT	&=	~BIT0;											// nRF24L01+ mode: Standby-I
	P1OUT	&=	~BIT1;											// nRF24L01+ mode: Standby-I
...
{
	uint8_t val [ 1 ] = {0x70};// cambio de valor de reset de 0x70 a 0x07
