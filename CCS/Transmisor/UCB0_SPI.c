...
uint8_t Write_Byte_UCB0_SPI (uint8_t Byte) // aca tengo que hacer el shift del registro, con USICNT
{
	uint8_t aux = 0;
	...
	/*//UCB0TXBUF   = 	 Byte;
	USISRL   = 	 Byte;
	while ( !( IFG2 & USISRL ) && !( IFG1 & WDTIFG ));		// USCI_B0 rX buffer ready?
...
*/
	...
	USICTL1 &= ~USIIFG;//pruebo limpiar la interrupcion para que non quede el pedido pendiente
	WDTCTL = WDTPW + WDTHOLD;     								// Stop watchdog timer
	aux			=	 USISRL;
